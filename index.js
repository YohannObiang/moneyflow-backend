...
app.post('/ajout/retrait', (req, res) => {
  const id = req.body.id;
    ...
    const toreceive = req.body.toreceive;
    const airtelmoney = req.body.airtelmoney;
    const rib = req.body.rib;
    const nom = req.body.nom;
    const bank = req.body.bank;

  con.query('INSERT INTO retraits VALUES(NULL, ?, ?, ?, ?, ?, ?, ?)', [date, tosend, toreceive, airtelmoney, rib, nom, bank], (err, result) => {
    if (err) {
      console.log(err);
    } else {
    const id_retrait = result.insertId; // Récupération de l'ID généré

      res.send({ id_retrait: id_retrait, date: date});
    }
  });
});
